...
func (nw *watcher) callBackHandle(services []model.Instance, err error) {
	...
	nw.RLock()
	cacheServeName := nw.cacheServices[serviceName]
	nw.RUnlock()

	if cacheServeName == nil {
		nw.Lock()
		...
		for _, subscribeService := range services {
			create := true
			nw.RLock()
			arr := nw.cacheServices[serviceName]
			nw.RUnlock()
			for _, cacheService := range arr {
		...
		arr := nw.cacheServices[serviceName]
		nw.RUnlock()
		for index, cacheService := range arr {
			del := true
			for _, subscribeService := range services {
				if subscribeService.InstanceId == cacheService.InstanceId {
					del = false
