...
func (a *AdminMenuReqValidator) EditReq(data interface{}) url.Values {
	...
	messages := govalidator.MapData{
		...
		validator.GetValidateJsonOmitemptyTag("path"):     []string{"required:接口路由不能为空", fmt.Sprintf("regex:%s", PatternAdminApiPathMsg)},
		validator.GetValidateJsonOmitemptyTag("key"):      []string{"required:接口路由键名不能为空", fmt.Sprintf("regex:%s", PatternAdminApiKeyMsg)},
		validator.GetValidateJsonOmitemptyTag("name"):     []string{"required:接口名称不能为空", fmt.Sprintf("regex:%s", PatternTrimBlankStringMsg)},
		validator.GetValidateJsonOmitemptyTag("describe"): []string{fmt.Sprintf("regex:%s", PatternTrimBlankStringMsg)},
	}
	opts := govalidator.Options{
		Data:     data,
		Rules:    rules,
...
func (a *AdminMenuReqValidator) ShowReq(data interface{}) url.Values {
	rules := govalidator.MapData{
		validator.GetValidateJsonOmitemptyTag("menuId"): []string{"required", "min:1"},
		validator.GetValidateJsonOmitemptyTag("field"):  []string{"required"},
		validator.GetValidateJsonOmitemptyTag("show"):   []string{"bool"},
	}
	messages := govalidator.MapData{
		validator.GetValidateJsonOmitemptyTag("menuId"): []string{"required:ID不能为空", "min:ID无效"},
		validator.GetValidateJsonOmitemptyTag("field"):  []string{"参数错误"},
		validator.GetValidateJsonOmitemptyTag("show"):   []string{"bool:类型错误"},
	}
	opts := govalidator.Options{
		Data:     data,
		Rules:    rules,
